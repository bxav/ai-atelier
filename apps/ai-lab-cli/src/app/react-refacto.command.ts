...
@Command({ name: 'react-refacto', description: 'React Refacto' })
export class ReactRefactoCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    ...
    let newFileContents = await chain.batch(
    ...
    let newFileContentsDict = Object.entries(fileContents).reduce(
      (acc, [path], index) => {
